...
// [[Rcpp::export]]
Rcpp::List admm_genlasso_aug(const arma::mat &A, const arma::colvec &b,
...
Rcpp::List admm_genlasso_for_graph(const arma::mat &A, const arma::colvec &b,
  ...
  arma::colvec z1 = z.elem(index1); 
  arma::colvec z2 = z.elem(index2);
  arma::colvec u_prev(D * x - z);
  arma::colvec u(D * x - z);
  ...
  int k;
  for (k = 0; k < maxiter; k++) {
    arma::colvec u1 = u.elem(index1);  
    arma::colvec u2 = u.elem(index2);
    // 4-1. update 'x'
    q = Atb - D.t() * (2 * u - u_prev) + rho / 2 * M * x +
        rho / 2 * M.t() * x; // temporary value
    x = solve(trimatu(U), solve(trimatl(L), q));
